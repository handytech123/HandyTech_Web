...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Enhanced chatbot endpoint with intelligent fallback system and improved live handoff
    ...
    try {
      ...
      if (isHandoffRequest) {
        ...
        liveChatSessions.set(sessionId, {
          isLive: false,
          pendingHandoff: true,
          messages: [{ type: 'customer', message, timestamp: new Date() }],
          createdAt: new Date()
        });
        
        // Send notifications via SMS and email
        try {
          await notificationService.notifyHandoffRequest(sessionId, message);
        } catch (error) {
          console.error('Failed to send handoff notifications:', error);
        }
        
        return res.json({
          conversationId: sessionId,
          response: "Got it — I've alerted a team member to join this chat. Someone will be with you shortly!",
