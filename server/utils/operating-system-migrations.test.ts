...
for(const sourceTable of ["customers","quotes","consultations","appointments","quote_proposals","jobs","referral_leads","job_expenses","change_orders","invoices","invoice_payments","project_gallery","reviews","chat_conversations","chat_messages"]){assert.equal(source.includes(`'${sourceTable}'`),true,`Legacy source is not represented in the reconciliation ledger: ${sourceTable}`);}
