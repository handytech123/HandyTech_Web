    ...
    <div className="min-h-screen bg-light-gray">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="space-y-12">
          {projectsData?.enabled && <><PortalProjectMessage projects={projects}/><PortalProjectsSection projects={projects} /></>}
...
function PortalProjectMessage({projects}:{projects:PortalProjectsData["projects"]}) {
  const {toast}=useToast();const [projectId,setProjectId]=useState("");const [message,setMessage]=useState("");
  const send=useMutation({mutationFn:()=>apiRequest(`/api/portal/projects/${projectId}/messages`,"POST",{message}),onSuccess:()=>{setMessage("");queryClient.invalidateQueries({queryKey:["/api/portal/projects"]});toast({title:"Message sent",description:"HandyTech will see it with your project history."});},onError:(error:Error)=>toast({title:"Message not sent",description:error.message,variant:"destructive"})});
  if(!projects.length)return null;
  return <Card className="mb-6"><CardHeader><CardTitle className="flex items-center gap-2"><Mail className="h-5 w-5"/>Message HandyTech</CardTitle></CardHeader><CardContent className="space-y-3"><Select value={projectId} onValueChange={setProjectId}><SelectTrigger><SelectValue placeholder="Choose a project"/></SelectTrigger><SelectContent>{projects.map(project=><SelectItem key={project.id} value={String(project.id)}>{project.job_number} · {project.title}</SelectItem>)}</SelectContent></Select><Input value={message} onChange={event=>setMessage(event.target.value)} placeholder="Question or update about this project"/><Button disabled={!projectId||message.trim().length<2||send.isPending} onClick={()=>send.mutate()}><Mail className="mr-2 h-4 w-4"/>Send Message</Button></CardContent></Card>;
}
