...
type ProposalData = {
  ...
  proposal: { quoteNumber: string; lineItems: Array<{ description: string; quantity: number; rate: number; details?: string; estimatedHours?: string; materials?: string }>; discount: number; taxRate: number; subtotal: number; tax: number; total: number; notes?: string; validUntil: string; status: string; sentAt: string; respondedAt?: string; signerName?: string };
};
...
const parseQuoteSections = (notes = "") => {
  ...
  return { structured: matches.length > 0, sections };
};
  ...
  return <div className="min-h-screen bg-slate-100 text-slate-900 print:bg-white">
    ...
    <main className="mx-auto max-w-5xl px-4 py-8">
      {finalStatus ? <section className="rounded-2xl bg-white p-8 text-center shadow-sm">
        ...
        {quoteContent.structured && <section className="mt-6 space-y-6 rounded-2xl bg-white p-6 shadow-sm"><div><div className="text-xs font-bold uppercase tracking-wider text-blue-700">Project overview</div><p className="mt-2 whitespace-pre-wrap leading-7 text-slate-700">{quoteContent.sections["PROJECT SUMMARY"]}</p></div><div className="grid gap-6 md:grid-cols-2"><div className="rounded-xl border border-emerald-200 bg-emerald-50/50 p-5"><h2 className="font-bold text-emerald-950">What is included</h2><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-700">{quoteContent.sections["INCLUDED WORK"]}</p></div>{quoteContent.sections["EXCLUSIONS & ASSUMPTIONS"] && <div className="rounded-xl border border-amber-200 bg-amber-50/50 p-5"><h2 className="font-bold text-amber-950">Exclusions and assumptions</h2><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-700">{quoteContent.sections["EXCLUSIONS & ASSUMPTIONS"]}</p></div>}</div><div className="grid gap-4 border-t pt-5 sm:grid-cols-2"><div><div className="text-xs font-bold uppercase text-slate-500">Estimated duration</div><p className="mt-1 font-medium">{quoteContent.sections["ESTIMATED DURATION"]}</p></div><div><div className="text-xs font-bold uppercase text-slate-500">Payment terms</div><p className="mt-1 text-sm leading-6">{quoteContent.sections["PAYMENT TERMS"]}</p></div></div>{quoteContent.sections["WORKMANSHIP"] && <div><div className="text-xs font-bold uppercase text-slate-500">Workmanship</div><p className="mt-1 text-sm leading-6 text-slate-700">{quoteContent.sections["WORKMANSHIP"]}</p></div>}</section>}
        <section className="mt-6 overflow-hidden rounded-2xl bg-white shadow-sm"><div className="border-b px-6 py-5"><h2 className="text-xl font-bold">{fixedProjectPrice ? "Project phases" : "Detailed pricing"}</h2><p className="mt-1 text-sm text-slate-600">{fixedProjectPrice ? "These phases are included together in the fixed project price and are not individually priced or removable." : "The following work and charges make up your quoted total."}</p></div>{fixedProjectPrice ? <div className="divide-y">{proposal.lineItems.map((item, index) => <div key={index} className="flex gap-4 p-5"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-blue-100 font-bold text-blue-700">{index + 1}</div><div><h3 className="font-bold">{item.description}</h3>{item.details && <p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-slate-600">{item.details}</p>}<div className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-xs text-slate-500">{item.estimatedHours && <span><strong>Approximate labor effort:</strong> {item.estimatedHours}</span>}{item.materials && <span><strong>Materials included:</strong> {item.materials}</span>}</div></div></div>)}</div> : <div className="overflow-x-auto"><table className="w-full"><thead className="bg-blue-50 text-left text-xs uppercase tracking-wider"><tr><th className="p-4">Description</th><th className="p-4 text-right">Qty</th><th className="p-4 text-right">Rate</th><th className="p-4 text-right">Amount</th></tr></thead><tbody>{proposal.lineItems.map((item, index) => <tr key={index} className="border-t"><td className="p-4"><div className="font-medium">{item.description}</div>{item.details && <div className="mt-1 text-xs text-slate-500">{item.details}</div>}</td><td className="p-4 text-right">{item.quantity}</td><td className="p-4 text-right">{money(item.rate)}</td><td className="p-4 text-right">{money(item.quantity * item.rate)}</td></tr>)}</tbody></table></div>}
          <div className="ml-auto w-full max-w-sm space-y-2 p-6"><div className="flex justify-between"><span>{fixedProjectPrice ? "Fixed project price" : "Subtotal"}</span><span>{money(proposal.subtotal)}</span></div>{proposal.discount > 0 && <div className="flex justify-between"><span>Discount</span><span>-{money(proposal.discount)}</span></div>}{proposal.tax > 0 && <div className="flex justify-between"><span>Tax ({proposal.taxRate}%)</span><span>{money(proposal.tax)}</span></div>}<div className="flex justify-between border-t-2 border-blue-600 pt-3 text-xl font-bold"><span>Total investment</span><span>{money(proposal.total)}</span></div></div>
          {proposal.notes && <div className="border-t bg-slate-50 p-6"><h2 className="font-bold">Additional notes</h2><p className="mt-2 whitespace-pre-wrap text-sm text-slate-600">{quoteContent.structured ? quoteContent.sections["ADDITIONAL NOTES"] || "Changes outside this quote require approval before work proceeds." : proposal.notes}</p></div>}
        </section>
