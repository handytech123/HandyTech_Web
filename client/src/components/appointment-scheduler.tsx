...
export default function AppointmentScheduler({ defaultBookingMode = false, defaultServiceName, defaultServiceId }: AppointmentSchedulerProps) {
  ...
  const servicesByCategory = activeServices.reduce((acc, service) => {
    if (!acc[service.category as CategoryKey]) acc[service.category as CategoryKey] = [];
  ...
  useEffect(() => {
    if (defaultBookingMode && defaultServiceName && activeServices.length > 0 && !selectedService) {
      const matchedService = activeServices.find(service => service.name === defaultServiceName);
      if (matchedService) {
        setSelectedService(matchedService);
        form.setValue("serviceType", matchedService.name);
      }
    }
  }, [defaultBookingMode, defaultServiceName, activeServices, selectedService, form]);

  useEffect(() => {
    if (defaultBookingMode && defaultServiceId && activeServices.length > 0 && !selectedService) {
      const matchedService = activeServices.find(service => service.id === defaultServiceId);
      if (matchedService) {
        setSelectedService(matchedService);
        form.setValue("serviceType", matchedService.name);
      }
    }
  }, [defaultBookingMode, defaultServiceId, activeServices, selectedService, form]);

  const categoryFromService = (service: Service) => {
    if (service.category === "essential") return "A" as const;
    if (service.category === "improvement") return "B" as const;
    return "C" as const;
  };

  const selectService = (service: Service) => {
    setSelectedService(service);
    setSelectedCategory(categoryFromService(service));
    form.setValue("serviceType", service.name);
  };

  const handleQuickPickSelect = (service: Service) => {
    selectService(service);
    setCurrentStep("contact");
    setTimeout(() => document.querySelector('[data-testid="card-contact-details"]')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 100);
  };
  ...
  const bookAppointmentMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Appointment Booked!", description: "Your appointment has been successfully scheduled. You'll receive a confirmation email shortly." });
    },
    onError: (error: Error) => {
  ...
  const handleTimeSelect = (timeSlot: string) => {
    ...
    setTimeout(() => document.querySelector('[data-testid="card-appointment-summary"]')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 100);
  };

  const onSubmit = (data: BookingFormData) => bookAppointmentMutation.mutate(data);

  const isReadyToSubmit = () => {
    const formData = form.getValues();
    return selectedService && selectedDate && selectedTimeSlot && formData.firstName && formData.lastName && formData.email && formData.phone && !form.formState.errors.firstName && !form.formState.errors.lastName && !form.formState.errors.email && !form.formState.errors.phone;
  };
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <Card className="w-full max-w-md mx-auto">
            <CardContent className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <Button onClick={() => { setIsSubmitted(false); setSelectedCategory(undefined); setSelectedService(undefined); setSelectedDate(undefined); setSelectedTimeSlot(""); setCurrentStep("category"); }}>
  ...
  const categoryLabel = (category: CategoryKey) => SERVICE_CATEGORIES[category].title;

  return (
    <section id="scheduler" className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8">
          <div className="bg-light-gray text-charcoal px-4 py-2 rounded-full text-sm font-semibold inline-block mb-4">SCHEDULE SERVICE</div>
          <h2 className="text-4xl font-bold text-charcoal mb-3">Book Your <span className="text-brand-red">HandyTech Appointment</span></h2>
          <p className="text-lg text-gray-600 max-w-3xl mx-auto">Choose a time block that fits your project. Materials billed separately. Travel included within 20 miles of Hazelwood, MO.</p>
        </div>
        ...
        <div className="space-y-6 mb-8">
          <div>
            ...
            {loadingServices ? (
              <div className="text-sm text-gray-500">Loading popular services...</div>
              ...
              <div className="flex flex-wrap gap-2">
                {quickPickServices.map((service) => (
                  <button
                    key={service.id}
                    type="button"
                    onClick={() => handleQuickPickSelect(service)}
                    className="inline-flex items-center gap-2 rounded-full border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-charcoal hover:border-brand-red hover:text-brand-red"
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold uppercase tracking-wide text-charcoal">A / B / C Service Menu</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {(Object.keys(SERVICE_CATEGORIES) as CategoryKey[]).map((category) => {
                ...
                return (
                  <div key={category} className="rounded-xl border border-gray-200 bg-white p-4">
                    <h4 className="text-sm font-semibold text-charcoal mb-2">{SERVICE_CATEGORIES[category].title}</h4>
                    <div className="space-y-2">
                      {servicesInCategory.length === 0 ? (
                        <p className="text-sm text-gray-500">No active services in this category yet.</p>
                      ) : (
                        servicesInCategory.map((service) => (
                          <button
                            ...
                            data-testid={`category-service-${service.id}`}
                          >
        ...
        <div className="space-y-6">
          {currentStep === "category" && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                {(Object.entries(SERVICE_CATEGORIES) as [CategoryKey, typeof SERVICE_CATEGORIES[CategoryKey]][]).map(([key, category]) => {
                  const IconComponent = category.icon;
                  const categoryServices = servicesByCategory[key] || [];
                  return (
                    <Card key={key} className="bg-white border-2 border-gray-100 rounded-xl">
                      <CardContent className="p-5">
                        <div className="flex items-center mb-3">
                          <div className="bg-light-gray w-11 h-11 rounded-full flex items-center justify-center mr-3">
                            <IconComponent className="text-charcoal" size={20} />
                          </div>
                          <div>
                            <h3 className="text-base font-bold text-charcoal">{category.title}</h3>
                          </div>
                        </div>
                        <ul className="text-sm text-gray-600 space-y-1">
                          {categoryServices.length === 0 ? (
                            <li>No active services in this category yet.</li>
                          ) : (
                            categoryServices.map((service) => (
                              <li key={service.id} className="flex items-center justify-between gap-2">
                                <button type="button" onClick={() => handleQuickPickSelect(service)} className="text-left hover:text-brand-red">
                                  {service.name}
                                </button>
                                <span className="text-xs text-gray-500">{service.basePrice ? `$${service.basePrice}` : ""}</span>
                              </li>
              ...
              <CardContent className="space-y-5">
                <div className="rounded-xl border border-gray-200 bg-gray-50 p-4">
                  ...
                  <div className="flex flex-wrap gap-2 items-center">
                    {selectedService ? (
                      <>
                        <Badge variant="outline" className="bg-white border-brand-red text-brand-red">{selectedService.name}</Badge>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <FormItem className="md:col-span-2">
                      ...
                      <Select onValueChange={(value) => {
                        ...
                        if (service) {
                          selectService(service);
                          if (defaultBookingMode && currentStep === "contact") return;
                        }
                        ...
                        <SelectContent>
                          {activeServices.map((service) => (<SelectItem key={service.id} value={service.id.toString()}>{service.name} ({service.suggestedHours}h)</SelectItem>))}
                        </SelectContent>
                      </Select>
                    </FormItem>
                    ...
                    <Button onClick={() => {
                      ...
                      if (formData.firstName && formData.lastName && formData.email && selectedService) {
                        ...
                        if (isValid) setCurrentStep("date");
                        else form.trigger(["firstName", "lastName", "email"]);
                      }
                    }} disabled={!form.watch("firstName") || !form.watch("lastName") || !form.watch("email") || !selectedService} className="w-full bg-brand-red hover:bg-red-700 md:col-span-2" data-testid="button-proceed-to-date">
                      Continue to Date Selection <ArrowRight className="ml-2 h-4 w-4" />
                    </Button>
                  </div>
                </Form>
              </CardContent>
          ...
          {currentStep === "date" && (<Card data-testid="card-date-selection"><CardHeader><CardTitle className="flex items-center gap-2"><CalendarDays className="h-5 w-5 text-brand-red" />Step 3: Select Date</CardTitle></CardHeader><CardContent><div className="mb-4"><Badge variant="outline" className="mb-2">Selected: {selectedService?.name} ({selectedService?.suggestedHours}h)</Badge></div><Calendar mode="single" selected={selectedDate} onSelect={handleDateSelect} disabled={(date) => date < addHours(new Date(), 12) || date.getDay() === 0} className="rounded-md border w-full" data-testid="calendar-date-picker" /><p className="text-sm text-gray-500 mt-2">Appointments must be booked at least 12 hours in advance. Sundays are not available.</p></CardContent></Card>)}

          {currentStep === "time" && (<Card data-testid="card-time-selection"><CardHeader><CardTitle className="flex items-center gap-2"><Clock className="h-5 w-5 text-brand-red" />Step 4: Choose Time</CardTitle></CardHeader><CardContent><div className="mb-4 space-y-2"><Badge variant="outline">Service: {selectedService?.name} ({selectedService?.suggestedHours}h)</Badge><Badge variant="outline">Date: {selectedDate && format(selectedDate, "EEEE, MMMM do, yyyy")}</Badge></div>{loadingSlots ? (<div className="flex items-center justify-center py-8"><Loader2 className="h-6 w-6 animate-spin text-brand-red" /><span className="ml-2 text-gray-600">Loading available times...</span></div>) : slotsError ? (<Alert variant="destructive"><AlertCircle className="h-4 w-4" /><AlertDescription>Failed to load available times. Please try selecting a different date.</AlertDescription></Alert>) : formattedSlots.length === 0 ? (<Alert><AlertCircle className="h-4 w-4" /><AlertDescription>No available times for this date. Please try a weekday (Monday-Friday) between now and next month. If you continue to see this message, please contact us directly.</AlertDescription></Alert>) : (<div className="grid grid-cols-2 sm:grid-cols-3 gap-2">{formattedSlots.map((slot) => (<Button key={slot.time} onClick={() => handleTimeSelect(slot.time)} variant="outline" className="w-full hover:bg-brand-red hover:text-white" data-testid={`button-time-${slot.displayTime.replace(/[^\w]/g, '-')}`}>{slot.displayTime}</Button>))}</div>)}</CardContent></Card>)}
        </div>
      </div>
    </section>
  );
