        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="rounded-lg border p-4 sm:col-span-2">
            <div className="flex items-start gap-3">
              <CalendarClock className="mt-0.5 h-5 w-5 text-brand-red" />
              <div>
          ...
          <a href={`tel:${appointment.phone || ""}`} className={`rounded-lg border p-4 transition-colors ${appointment.phone ? "hover:bg-muted" : "pointer-events-none opacity-60"}`}>
            <div className="flex items-center gap-3"><Phone className="h-5 w-5 text-brand-red" /><div><p className="text-xs text-muted-foreground">Call customer</p><p className="font-medium">{appointment.phone || "No phone provided"}</p></div></div>
          </a>
          <a href={`mailto:${appointment.email}`} className="rounded-lg border p-4 transition-colors hover:bg-muted">
            <div className="flex items-center gap-3"><Mail className="h-5 w-5 text-brand-red" /><div className="min-w-0"><p className="text-xs text-muted-foreground">Email customer</p><p className="truncate font-medium">{appointment.email}</p></div></div>
          </a>

          <div className="rounded-lg border p-4 sm:col-span-2">
            <div className="flex items-start gap-3"><Wrench className="mt-0.5 h-5 w-5 text-brand-red" /><div><p className="text-xs text-muted-foreground">Service</p><p className="font-semibold">{appointment.serviceType}</p></div></div>
          </div>

          <div className="rounded-lg border p-4 sm:col-span-2">
            <div className="flex items-start justify-between gap-4">
              <div className="flex min-w-0 items-start gap-3"><MapPin className="mt-0.5 h-5 w-5 shrink-0 text-brand-red" /><div><p className="text-xs text-muted-foreground">Service address</p><p className="font-medium">{address || "No address provided"}</p></div></div>
