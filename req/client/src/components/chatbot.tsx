...
export default function Chatbot() {
  ...
  const [position, setPosition] = useState({ x: 24, y: 24 }); // Default bottom-6 right-6 (24px)
  ...
  // Initialize session ID and position when component mounts
  useEffect(() => {
    getSessionId();
  ...
  const handleMouseMove = (e: MouseEvent) => {
    ...
    setPosition({
      x: window.innerWidth - newX - 64, // Convert to right offset
      y: window.innerHeight - newY - 64  // Convert to bottom offset
    });
  };
  ...
  // Drag handlers for touch events (mobile)
    ...
    <>
      ...
      >
        <Button
          ...
          className={`w-16 h-16 rounded-full bg-brand-red hover:bg-brand-red-dark shadow-lg select-none ${isDragging ? 'cursor-grabbing' : 'cursor-grab'}`}
          size="icon"
        >
